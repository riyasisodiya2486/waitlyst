...
import { suggestRewardTiers } from '@/lib/api-client'
...
function MetricSkeleton() {
  return (
    <div className="animate-pulse rounded-[12px] border border-[rgba(255,255,255,0.06)] bg-[#0F0F0F] p-6">
      ...
      <div className="mt-4 h-10 w-20 rounded bg-[rgba(200,241,53,0.12)]" />
    </div>
  )
}

export function DashboardPageClient({ initialCampaigns }: { initialCampaigns: Campaign[] }) {
  ...
  async function handleCreateCampaign(e: React.FormEvent) {
    ...
    try {
      ...
      const response = await fetch('/api/campaigns', {
        ...
        body: JSON.stringify({ title, description, rewardTiers }),
      })
      ...
      <div className="px-4 pb-28 pt-20 md:ml-14 md:px-6 md:pb-8 lg:ml-[200px] lg:px-8">
        <div className="mx-auto max-w-[1200px] space-y-8">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h1 className="instrument-serif text-[28px] sm:text-[32px]">Campaigns</h1>
              <p className="mt-2 text-[14px] text-[#8A8782]">Create a campaign, share the public link, and track real signups.</p>
            </div>
            <button onClick={() => setShowForm((prev) => !prev)} className="w-full rounded bg-[#C8F135] px-4 py-3 text-[13px] font-medium text-[#080808] sm:w-auto">
            ...
            {loading ? (
              ...
              <>
                <div className="rounded-[12px] border border-[rgba(255,255,255,0.06)] bg-[#0F0F0F] p-6">
                  ...
                  <div className="mt-2 dm-mono text-[32px] text-[#C8F135] sm:text-[36px]">{totalSignups}</div>
                </div>
                <div className="rounded-[12px] border border-[rgba(255,255,255,0.06)] bg-[#0F0F0F] p-6">
                  <div className="dm-mono text-[12px] uppercase text-[#5C5955]">Campaigns</div>
                  <div className="mt-2 dm-mono text-[32px] text-[#C8F135] sm:text-[36px]">{campaigns.length}</div>
                </div>
                <div className="rounded-[12px] border border-[rgba(255,255,255,0.06)] bg-[#0F0F0F] p-6">
                  <div className="dm-mono text-[12px] uppercase text-[#5C5955]">Avg Referral Rate</div>
                  <div className="mt-2 dm-mono text-[32px] text-[#C8F135] sm:text-[36px]">{avgReferralRate}%</div>
                </div>
              </>
            )}
          </div>
          ...
          {showForm && (
            <form onSubmit={handleCreateCampaign} className="space-y-4 rounded-[12px] border border-[rgba(255,255,255,0.06)] bg-[#0F0F0F] p-4 sm:p-6">
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                <label className="space-y-2">
                  <span className="text-[13px] text-[#8A8782]">Campaign title</span>
                  <input value={title} onChange={(e) => setTitle(e.target.value)} className="w-full rounded border border-[rgba(255,255,255,0.1)] bg-[#080808] px-4 py-3 text-[14px] outline-none" placeholder="Waitlyst Pro Launch" required />
                </label>
                <div className="flex items-end">
                  <button type="button" onClick={handleSuggestTiers} disabled={!description.trim() || suggesting} className="w-full rounded border border-[rgba(200,241,53,0.35)] px-4 py-3 text-[13px] text-[#C8F135] disabled:opacity-50 md:w-auto">
              ...
              <label className="block space-y-2">
                ...
                <textarea value={description} onChange={(e) => setDescription(e.target.value)} className="min-h-28 w-full rounded border border-[rgba(255,255,255,0.1)] bg-[#080808] px-4 py-3 text-[14px] outline-none" placeholder="Describe what people are joining for." />
              </label>
              ...
              <table className="w-full min-w-[720px] text-left text-sm">
                <thead>
                  <tr className="border-b border-[rgba(255,255,255,0.06)] text-[12px] uppercase text-[#5C5955]">
                    ...
                    <th className="px-4 py-4 sm:px-6">Referral Rate</th>
                ...
                <tbody>
                  {loading ? (
                    Array.from({ length: Math.max(campaigns.length || 0, 3) }).map((_, index) => (
                      <tr key={index} className="animate-pulse border-b border-[rgba(255,255,255,0.04)]">
                        <td className="px-4 py-4 sm:px-6"><div className="h-4 w-32 rounded bg-[rgba(255,255,255,0.08)]" /></td>
                        <td className="px-4 py-4 sm:px-6"><div className="h-4 w-28 rounded bg-[rgba(255,255,255,0.08)]" /></td>
                        <td className="px-4 py-4 sm:px-6"><div className="h-4 w-12 rounded bg-[rgba(255,255,255,0.08)]" /></td>
                        <td className="px-4 py-4 sm:px-6"><div className="h-4 w-16 rounded bg-[rgba(200,241,53,0.12)]" /></td>
                        <td className="px-4 py-4 sm:px-6"><div className="h-4 w-16 rounded bg-[rgba(255,255,255,0.08)]" /></td>
                        <td className="px-4 py-4 sm:px-6"><div className="h-4 w-20 rounded bg-[rgba(255,255,255,0.08)]" /></td>
                      </tr>
                    ))
                  ) : !campaigns.length ? (
                    <tr>
                      <td colSpan={6} className="px-4 py-8 text-[14px] text-[#8A8782] sm:px-6">No campaigns yet. Create one to start testing the public signup flow.</td>
                    </tr>
                  ) : (
                    campaigns.map((campaign) => (
                      <tr key={campaign.id} className="border-b border-[rgba(255,255,255,0.04)] text-[13px]">
                        <td className="px-4 py-4 text-[#F0EDE6] sm:px-6">{campaign.name}</td>
                        <td className="px-4 py-4 dm-mono text-[#8A8782] sm:px-6">{campaign.slug}</td>
                        <td className="px-4 py-4 dm-mono text-[#F0EDE6] sm:px-6">{campaign.signups}</td>
                        <td className="px-4 py-4 dm-mono text-[#C8F135] sm:px-6">{campaign.referralRate}%</td>
                        <td className="px-4 py-4 text-[#8A8782] sm:px-6">{campaign.status}</td>
