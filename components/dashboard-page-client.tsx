...
export function DashboardPageClient({ initialCampaigns }: { initialCampaigns: Campaign[] }) {
  ...
  const avgReferralRate =
    campaigns.length > 0 ? Math.round(campaigns.reduce((sum, campaign) => sum + (campaign.referralRate || 0), 0) / campaigns.length) : 0
      ...
      <div className="ml-14 px-8 pt-20">
        <div className="mx-auto max-w-[1200px] space-y-8">
          <div className="flex items-center justify-between gap-4">
            ...
            <button onClick={() => setShowForm((prev) => !prev)} className="rounded bg-[#C8F135] px-4 py-3 text-[13px] font-medium text-[#080808]">
            ...
            <form onSubmit={handleCreateCampaign} className="space-y-4 rounded-[12px] border border-[rgba(255,255,255,0.06)] bg-[#0F0F0F] p-6">
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <div className="flex items-end">
                  <button type="button" onClick={handleSuggestTiers} disabled={!description.trim() || suggesting} className="rounded border border-[rgba(200,241,53,0.35)] px-4 py-3 text-[13px] text-[#C8F135] disabled:opacity-50">
                ...
                <tbody>
                  {loading ? (
                    Array.from({ length: Math.max(campaigns.length || 0, 3) }).map((_, index) => (
                      <tr key={index} className="border-b border-[rgba(255,255,255,0.04)] animate-pulse">
                        <td className="px-6 py-4"><div className="h-4 w-32 rounded bg-[rgba(255,255,255,0.08)]" /></td>
                        <td className="px-6 py-4"><div className="h-4 w-28 rounded bg-[rgba(255,255,255,0.08)]" /></td>
                        <td className="px-6 py-4"><div className="h-4 w-12 rounded bg-[rgba(255,255,255,0.08)]" /></td>
                        <td className="px-6 py-4"><div className="h-4 w-16 rounded bg-[rgba(200,241,53,0.12)]" /></td>
                        <td className="px-6 py-4"><div className="h-4 w-16 rounded bg-[rgba(255,255,255,0.08)]" /></td>
                        <td className="px-6 py-4"><div className="h-4 w-20 rounded bg-[rgba(255,255,255,0.08)]" /></td>
                      </tr>
                    ))
                  ) : !campaigns.length ? (
                    <tr>
                      <td colSpan={6} className="px-6 py-8 text-[14px] text-[#8A8782]">No campaigns yet. Create one to start testing the public signup flow.</td>
                    </tr>
                  ) : (
